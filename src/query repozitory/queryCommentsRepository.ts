...
export class CommentsQueryRepository {
  ...
  async getAllCommentsForPost(
    ...
    const userReactionsMap = new Map(
      userReactions.map(reaction => [reaction.parentId.toString(), reaction.myStatus])
    );
    ...
    const mappedComments: CommentViewModel[] = result.map(
      (el: CommentsMongoDbType): CommentViewModel => ({
        ...
        likesInfo: {
          ...
          myStatus: userReactionsMap.get(el._id.toString()) || ReactionStatusEnum.None,
        },
      })
    );
  ...
  async findCommentById(id: string, userId?:string): Promise<CommentViewModel | null> {
...
let myStatus:ReactionStatusEnum = ReactionStatusEnum.None;
  ...
  async findCommentsByParentId(
    parentId: string,
    pagination: PaginatedType,
    userId: string // Добавим параметр userId для определения реакции конкретного пользователя
  ): Promise<Paginated<CommentViewModel>> {
    ...
    const mappedComments: CommentViewModel[] = result.map(
      (comment: CommentsMongoDbType): CommentViewModel => ({
        ...
        likesInfo: {
          likesCount: comment.likesInfo.likesCount,
          dislikesCount: comment.likesInfo.dislikesCount,
          myStatus: userReactionsMap.get(comment._id.toString()) || ReactionStatusEnum.None,
        }
      })
    );
